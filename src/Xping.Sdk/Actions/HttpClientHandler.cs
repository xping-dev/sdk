...
/// <summary>
/// The HttpClientHandler class is a concrete implementation of the <see cref="TestComponent"/> class that is 
/// used to send an HTTP request. It uses the <see cref="IHttpClientFactory"/> to create an instance of the 
...
/// method.
/// </remarks>
public sealed class HttpClientHandler : TestComponent
{
    private readonly OrderedUrlTracker _urlRedirections = [];
    private readonly HttpClientOptions _configuration;
    ...
    public HttpClientHandler(HttpClientOptions configuration) :
        base(nameof(HttpClientHandler), type: TestStepType.ActionStep)
    {
    ...
    private HttpClient CreateHttpClient(IHttpClientFactory httpClientFactory)
    {
        ...
        {
            httpClient = httpClientFactory.CreateClient(HttpClientFactoryOptions.HttpClientWithRetryPolicy);
        }
        else if (_configuration.RetryHttpRequestWhenFailed == false)
        {
            httpClient = httpClientFactory.CreateClient(HttpClientFactoryOptions.HttpClientWithNoRetryPolicy);
        }
